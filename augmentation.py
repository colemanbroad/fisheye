...
def unet_warp_channels(img, delta):
    a,b,c = img.shape
    stack = img.copy()
    for i in range(c):
...
def warp_all_possible(xs, ys):
    ...
    # m = random.random()*xs.mean()
    s = random.random()*xs.std()
    noise = np.random.normal(0,s/4,xs.shape).astype(xs.dtype)
    xs += noise
    xs = np.clip(xs, 0, None)

    xs = np.flip(xs, axis=1) # axis=1 is the horizontal axis
    ys = np.flip(ys, axis=1) # axis=1 is the horizontal axis

    sc = random.random()*5
    delta = np.random.normal(loc=0, scale=sc, size=(2,3,3))
    xs,_,_ = unet_warp_orig(xs, delta=delta)
    ys,_,_  = unet_warp_orig(ys, delta=delta, twolabel=True)

    randangle = (random.random()-0.5)*2*180
    xs  = rotate(xs, randangle, reshape=False)
    ys = rotate(ys, randangle, reshape=False)
    return xs, ys
